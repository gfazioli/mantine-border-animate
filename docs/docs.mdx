...
- **Custom color stops** - Multi-color gradients with `colorStops` for beam effects, rotating gradients, and more
...
The **beam** variant (default) creates a rotating conic-gradient light that sweeps around the border. The `size` prop controls how wide the beam wedge is — from a narrow spotlight to a full rotating gradient.

Key props for beam variant:

- `size` - Angular spread of the beam wedge (xs=5%, sm=10%, md=15%, lg=25%, xl=40%, or a number 0-50)
- `duration` - Controls how fast the beam rotates
- `colorFrom` / `colorTo` - Colors of the beam wedge
- `colorStops` - Full control over the gradient (overrides colorFrom/colorTo)

With `colorStops`, you can create any effect: a narrow comet trail, a rainbow rotating border, a half-and-half split, or anything in between.
...
The `colorStops` prop gives you full control over the beam's conic-gradient. Each stop has a `color` (any Mantine color) and a `position` (0-100%). When provided, it overrides `colorFrom`/`colorTo` and the `size` prop.

Use transparent stops to create beam/wedge effects, or fill the entire circle for a rotating gradient border (replacing the old `gradient` variant).
